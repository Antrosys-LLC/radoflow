...
**The live board is the floor at this minute, not the roster.** A check-in with
no check-out in the last sixteen hours is that person working now, on the date
it belongs to, whatever shift they are rostered to — at 08:15 that is the night
shift still in overtime and the day shift that has just arrived, and a
supervisor covering a night off his own roster is on the board the moment he
scans. Past sixteen hours it is a missed check-out, not a man on the floor.
Only when nothing is open does the roster decide, on the shift's own date —
yesterday's, for a shift that crossed midnight. The rule is written twice, in
the `live_attendance` view (20260922090000) and in `liveWorkDate`
(`src/lib/attendance/shift-now.ts`) for the dashboard's own card; change both
together or the two screens will disagree about the same person.
...
Every PDF and workbook closes on one sentence and nothing else — `This document
is electronically generated by an automated system for Rado Dyeing & Textile;
therefore, an official seal or signature is not required.` (`MACHINE_NOTICE`).
It replaced a footer carrying the period, the Sunday rule and a
"computer-generated" line, and the three signature rules a payslip used to
print: nobody signs these, and the period is already in the letterhead. Every
page is also watermarked with the factory's mark, painted through a fill alpha
(`Page.watermark`, `WATERMARK_ALPHA`) so it never competes with a figure.
