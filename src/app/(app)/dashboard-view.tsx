    ...
    <div className="space-y-5 pb-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatPill
          ...
          value={formatPKR(summary.budget)}
          hint={`${((summary.gross / summary.budget) * 100).toFixed(0)}% utilised`}
          tone="warning"
        />
        <StatPill
          icon={ClipboardCheck}
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <Row
              ...
              value={`- ${formatPKR(summary.statutory)}`}
              tone="danger"
            />
            ...
            <BarMeter
              value={(summary.gross / summary.budget) * 100}
              label="Budget utilisation"
              right={`${((summary.gross / summary.budget) * 100).toFixed(0)}%`}
            />
            <button
              disabled={!isExecutive}
              onClick={() =>
                toast.success("Payroll run queued for sign-off", { description: factoryName })
              }
            ...
            {!isExecutive ? (
              <p className="text-center text-xs text-muted-foreground">
                Sign-off is limited to Admin and CEO.
              </p>
            ) : null}
          </div>
        </Card>
      ...
      <Card>
        <SectionTitle
          ...
          subtitle="Present vs. roster strength"
        />
...
function Row({ label, value, tone }: { label: string; value: string; tone?: "danger" }) {
  return (
    <div className="flex items-center justify-between rounded-2xl bg-secondary px-4 py-3">
      ...
      <span
        className={
          tone === "danger" ? "text-sm font-bold text-danger" : "text-sm font-bold text-foreground"
        }
      >
        {value}
      </span>
