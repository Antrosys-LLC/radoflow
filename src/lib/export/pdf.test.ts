...
describe("what every document carries", () => {
  it("closes on the electronic-generation notice and nothing else", () => {
    const { text } = parse(buildTablePdf(table));

    expect(text).toContain("electronically generated by an automated system");
    expect(text).toContain("an official seal or signature is not required");
    // The period, the Sunday rule and the old machine line are all gone.
    expect(text).not.toContain("Sundays are not working days");
    expect(text).not.toContain("Computer-generated");
  });

  it("leaves no signature rules on a payslip", () => {
    const { text } = parse(
      buildPayslipPdf({
        employeeName: "ISMAIL KHAN",
        ...
        department: "Dyeing",
        period: "August 2026",
        facts: [{ label: "Daily rate", value: "1,600.00" }],
  ...
  it("marks every page with the factory's own logo, faintly", () => {
    const { text } = parse(buildTablePdf(table));

    // The mark painted through a fill alpha, and the alpha declared for it.
    expect(text).toContain("/GSW gs");
    expect(text).toContain("/Type /ExtGState");
    expect(text).toContain("/ExtGState << /GSW 7 0 R >>");
  });

  it("still points at the right objects with the watermark added", () => {
    const { entries, buf, size } = parse(
      buildPayslipPdf({
        employeeName: "MUSHTAQ",
        employeeCode: "RD-2070",
        department: "Finishing",
        period: "August 2026",
        facts: [{ label: "Working days", value: "25" }],
        earnings: [{ label: "Basic", amount: 30000 }],
        deductions: [{ label: "Advance", amount: 5000 }],
        net: 25000,
      }),
    );
    ...
    entries.forEach((entry, index) => {
      if (index === 0) return;
      const offset = Number(entry[1]);
      expect(buf.toString("latin1", offset, offset + 24).startsWith(`${index} 0 obj`)).toBe(true);
    });
  });
});
