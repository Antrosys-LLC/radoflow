...
function assemble(pages: Page[], title: string): Buffer {
  ...
  // 1 catalog, 2 pages, 3–4 fonts, 5–6 logo and its mask, 7 info, then pages.
  ...
  pages.forEach((page, i) => {
    ...
    objects[id] =
      ...
      `/Resources << /Font << /F1 3 0 R /F2 4 0 R >> /XObject << /Logo 5 0 R >> >> /Contents ${streamId} 0 R >>`;
    objects[streamId] =
...
export function buildTablePdf(doc: TableDoc): Buffer {
  ...
  const pages = chunks.map((chunk, pageIndex) => {
    ...
    pageFooter(page, doc.footer, pageIndex + 1, chunks.length);
    return page;
  });
...
export function buildPayslipPdf(slip: PayslipDoc): Buffer {
  ...
  // ---- Signatures -----------------------------------------------------------
  const sigY = Math.min(y - 30, MARGIN + 70);
  const sigW = (usable - 40) / 3;
  ["Prepared by", "Approved by", "Received by (employee)"].forEach((caption, i) => {
    const x = left + i * (sigW + 20);
    page.line(x, sigY, x + sigW, sigY, 0.35, 0.6);
    page.centreText(x + sigW / 2, sigY - 11, caption, 7.5, false, MUTED);
  });

  pageFooter(page, slip.footer ?? "Computer generated salary slip", 1, 1);
